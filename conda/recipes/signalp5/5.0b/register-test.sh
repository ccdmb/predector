if [ -z "${TEST_RETCODE:-}" ]
then
    echo "ERROR: Sorry the package author forgot to include a test so this now has to fail." 1>&2
    echo "ERROR: Please contact the maintainers <darcy.ab.jones@gmail.com>" 1>&2
    exit 1
fi
