...
We pin the version of the pipeline to run in all of our example commands with the `-r 1.2.5` parameter.
...
1. Provide the new version to the `-r` parameter, and add the `-latest` flag to tell Nextflow to pull new changes from the GitHub repository.
  ...
  ```
  nextflow run -r 1.2.5 -latest ccdmb/predector --proteomes "my_proteins.fasta"
  ```
...
```
nextflow run -profile docker -resume -r 1.2.5 ccdmb/predector \
