...
We pin the version of the pipeline to run in all of our example commands with the `-r 1.2.1-alpha` parameter.
...
1. Provide the new version to the `-r` parameter, and add the `-latest` flag to tell nextflow to pull new changes from the github repository.
  ...
  ```
  nextflow run -r 1.2.1-alpha -latest ccdmb/predector --proteomes "my_proteins.fasta"
  ```
