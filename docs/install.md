...
curl -s "https://raw.githubusercontent.com/ccdmb/predector/predector/predector:ccdmb/predector/-r 1.2.0-beta/install.sh" \
| bash -s "${ENVIRONMENT}" \
...
This will create the conda environment (named `predector`), or the docker (tagged `predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta`) or singularity (file `./predector.sif`) containers.
...
-n|--name      -- For conda, sets the environment name (default: 'predector').
                  For docker, sets the image tag (default: 'predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta').
...
```bash
nextflow run -profile test -with-conda /home/username/path/to/environment -resume -r predector/predector:ccdmb/predector/-r 1.2.0-beta ccdmb/predector
```

Using docker:

```bash
nextflow run -profile test,docker -resume -r predector/predector:ccdmb/predector/-r 1.2.0-beta ccdmb/predector

# if your docker configuration requires sudo use this profile instead
nextflow run -profile test,docker_sudo -resume -r predector/predector:ccdmb/predector/-r 1.2.0-beta ccdmb/predector
```

Using singularity:

```bash
nextflow run -profile test -with-singularity path/to/predector.sif -resume -r predector/predector:ccdmb/predector/-r 1.2.0-beta ccdmb/predector

# or if you've build the container using docker and it's in your local docker registry.
nextflow run -profile test,singularity -resume -r predector/predector:ccdmb/predector/-r 1.2.0-beta ccdmb/predector
```
...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/predector/predector:ccdmb/predector/-r 1.2.0-beta/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/predector/predector:ccdmb/predector/-r 1.2.0-beta/Dockerfile \
| docker build \
  ...
  -t predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta \
  -f - \
  .
```

Your container should now be available as `predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/predector/predector:ccdmb/predector/-r 1.2.0-beta/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta
```
...
docker save predector/predector:predector/predector:ccdmb/predector/-r 1.2.0-beta > predector.tar
```
