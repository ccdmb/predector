...
-n|--name      -- For conda, sets the environment name (default: 'predector').
                  For docker, sets the image tag (default: 'predector/predector:1.2.1-beta').
...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.2.1-beta/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.2.1-beta/Dockerfile \
| docker build \
  ...
  -t predector/predector:1.2.1-beta \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.2.1-beta` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.2.1-beta/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.2.1-beta
```
...
docker save predector/predector:1.2.1-beta > predector.tar
```
