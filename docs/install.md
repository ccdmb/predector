...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.2.1/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.2.1/Dockerfile \
| docker build \
  ...
  -t predector/predector:1.2.1 \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.2.1` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.2.1/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.2.1
```
