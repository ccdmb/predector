...
-n|--name         -- For conda, sets the environment name (default: 'predector').
                     For docker, sets the image tag (default: 'predector/predector:1.2.4-alpha').
...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.2.4-alpha/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.2.4-alpha/Dockerfile \
| docker build \
  ...
  --build-arg SIGNALP6=signalp-6.0d.fast.tar.gz \
  ...
  -t predector/predector:1.2.4-alpha \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.2.4-alpha` in your docker registry `docker images`.
...
export SIGNALP6=signalp-6.0d.fast.tar.gz
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.2.4-alpha/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.2.4-alpha
```
...
docker save predector/predector:1.2.4-alpha > predector.tar
```
