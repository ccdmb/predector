...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.2.0-beta/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.2.0-beta/Dockerfile \
| docker build \
  ...
  -t predector/predector:1.2.0-beta \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.2.0-beta` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.2.0-beta/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.2.0-beta
```
...
docker save predector/predector:1.2.0-beta > predector.tar
```
