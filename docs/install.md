...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.1.1-dev/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.1.1-dev/Dockerfile \
| docker build \
  ...
  -t predector/predector:1.1.1-dev \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.1.1-dev` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.1.1-dev/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.1.1-dev
```
...
docker save predector/predector:1.1.1-dev > predector.tar
```
