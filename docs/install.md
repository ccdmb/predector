...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/0.0.1-dev.3/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/0.0.1-dev.3/Dockerfile \
| docker build \
  ...
  -t predector/predector:0.0.1-dev.3 \
  -f - \
  .
```

Your container should now be available as `predector/predector:0.0.1-dev.3` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/0.0.1-dev.3/singularity.def
...
```bash
singularity build predector.sif docker://predector/predector:0.0.1-dev.3
```
...
docker save predector/predector:0.0.1-dev.3 > predector.tar
```
...
### Running with docker `Unable to find image 'predector/predector:0.0.1-dev.3' locally`
