...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/0.1.0-alpha/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/0.1.0-alpha/Dockerfile \
| docker build \
  ...
  -t predector/predector:0.1.0-alpha \
  -f - \
  .
```

Your container should now be available as `predector/predector:0.1.0-alpha` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/0.1.0-alpha/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:0.1.0-alpha
```
...
docker save predector/predector:0.1.0-alpha > predector.tar
```
