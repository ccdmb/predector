...
This will create the conda environment (named `predector`), or the docker (tagged `predector/predector:0.0.2-dev`) or singularity (file `./predector.sif`) containers.
...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/0.0.2-dev/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/0.0.2-dev/Dockerfile \
| docker build \
  ...
  -t predector/predector:0.0.2-dev \
  -f - \
  .
```

Your container should now be available as `predector/predector:0.0.2-dev` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/0.0.2-dev/singularity.def
...
```bash
singularity build predector.sif docker://predector/predector:0.0.2-dev
```
...
docker save predector/predector:0.0.2-dev > predector.tar
```
...
### Running with docker `Unable to find image 'predector/predector:0.0.2-dev' locally`
